...
import { CONFIG_FILENAME, CODFORM_ENV_PATTERN } from './constants.mjs'
import { logger } from './logger.mjs'
import { getAllRootFilesByFilepath, isRootFile, isRootPath, outputFileByFilepath } from './utils.mjs'
...
export function resolveSourceConfig (artifact, { filepath, config }) {
  const [, projectFolder] = filepath.split('\\')
  const rootPath = isRootPath(filepath)
  const rootFile = isRootFile(filepath)

  // falls back to any artifact with zero-config pattern
  // when no config.src is provided nor root file changes
  if (!rootFile && !config.src) {
    return getAllRootFilesByFilepath(filepath)
      .map(rootFilepath => ({
        entry: rootFilepath,
        output: outputFileByFilepath(rootFilepath)
      }))
  }

  const currentConfigKey = rootPath ? '__root__' : projectFolder
  const currentConfig = (config.src ?? {})[currentConfigKey]
  const artifactConfig = currentConfig?.[artifact]
  ...
  if (rootFile && !(currentConfig ?? {})[artifact]) {
    return [{
      entry: filepath,
      output: outputFileByFilepath(filepath)
    }]
  }
  ...
  return Array.isArray(artifactConfig)
    ? artifactConfig
    : [artifactConfig]
}

/**
 * Resolves the current codform according the current environment (NODE_ENV)
 * @param {Number|Object} rawCodform - current codform in pure number or an object with codform per environment
 * @param {String} env - current environment (default = local)
 * @returns {Number} codform
 */
export function resolveCodformConfig (rawCodform, env = 'local') {
  const codform = !isObject(rawCodform)
    ? rawCodform
    : rawCodform[env] || rawCodform[CODFORM_ENV_PATTERN[env]]

  if (!Number.isInteger(codform) || !codform) return
